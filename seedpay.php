...
/**
 * Submit initial request to seedpay
 */
...
function ajax_seedpay_check_request()
{
                ...
                if ($phone != '') {
                                ...
                                $getVars            = htmlentities(urlencode(json_encode(array(
                                                'uniqueTransactionId' => $transaction_id
                                ))));
...
/**
 * Generate a unique id since we can't get the cart id at this point
 */
...
/**
 * Add a settings link in the plugin settings
 */
...
/**
 * Make sure we can't add any new products once payment was accepted
 */
